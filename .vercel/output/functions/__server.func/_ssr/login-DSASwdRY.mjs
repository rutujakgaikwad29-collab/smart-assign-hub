...
import { b as Route$k, u as useAuth, i as isFirebaseConfigured, f as firebaseConfigMessage, g as getFirebaseAuthErrorMessage } from "./router-DpWtc24d.mjs";
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (userRole !== role) {
        ...
        setError(`Access denied. Your account is registered as "${roleLabel}" but you are trying to log in as "${attemptedLabel}". Please use the correct login portal.`);
        setLoading(false);
        return;
      }
      navigate({
